...
recheck:
	...
	if (policy < 0) {
		...
		if (!valid_policy(policy)
			...
			printk("We are in if !valid_policy\n");
			return -EINVAL;
		}
	}
	...
	if ((p->mm && attr->sched_priority > MAX_USER_RT_PRIO-1) ||
		...
		printk("we are in if p->mm && attr->sched_prio > MAX_USER\n");
		return -EINVAL;
}
	if ((dl_policy(policy) && !__checkparam_dl(attr)) ||
	    (rt_policy(policy) != (attr->sched_priority != 0))){
		printk("We are in if dl_policy(policy) && !__cgeck\n");
		printk("val of dl_policy is %d\n",dl_policy(policy));
		printk("val of rt_policy is %d\n",rt_policy(policy));
		printk("val of attr->sched_priority is %d\n",attr->sched_priority);
		return -EINVAL;
			       ...
			       unsigned int, flags)
{
	...
	if (!uattr || pid < 0 || flags){
		printk("We are in !uattr || pid < 0 || flasgs\n");
		return -EINVAL;
}

	retval = sched_copy_attr(uattr, &attr);
	if (retval){
		printk("We are in retval return\n");
		return retval;
}
	if ((int)attr.sched_policy < 0){
		printk("We are in (int)attr.sched_policy < 0\n");
		return -EINVAL;
}
