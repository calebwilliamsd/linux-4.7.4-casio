...
static struct task_struct *pick_next_task_casio(struct rq *rq, struct task_struct *prev, struct pin_cookie cookie)
{
...
{
	struct casio_task *t=NULL;
	printk("If this is printed, that means enqueue for casio task was entered\n");
	char msg[CASIO_MSG_SIZE];
	if(p){
		t=find_casio_task_list(&rq->casio_rq,p);
		if(t){
			printk("If this is printed, that means enqueue has entered into real enqueu\n");
...
static int
select_task_rq_casio(struct task_struct *p, int cpu, int sd_flag, int flags)
{
return 0;
}
